...
public class EquipoJpaController implements Serializable {
    ...
    public List<String> listaIdJugadores(Integer id)
    {
       EntityManager em = getEntityManager();
       Query query= em.createQuery("SELECT j.idjugador,concat(p.nombre,' ', p.apellidos), j.numerojugador \n" +
        "FROM Jugador j, Persona p \n" +
        "WHERE j.idequipo.idequipo = :id and j.idjugador = p.idpersona");
